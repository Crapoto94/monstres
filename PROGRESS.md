...
Dernière mise à jour : **2026-08-04** (v1.0.32 — alerte SMS admin à chaque nouvel inscrit)

### Récap v1.0.31 → v1.0.32 (2026-08-04)
- **v1.0.32** : `AuthService.notifyAdminOfNewUser()` (`backend/src/auth/auth.service.ts`) envoie désormais aussi un SMS à l'admin à chaque nouvelle inscription, en plus de l'email existant — deux nouveaux `settings` : `new_user_admin_sms_notification_enabled` (BOOLEAN, activé par défaut, toggle dans Admin > Paramètres > 🔔 Notifications) et `admin_notification_phone` (STRING, défaut `+33650175343`, éditable). N'empêche jamais l'inscription en cas d'échec (même filet que l'email). Seedés dans `backend/scripts/seed.js` (idempotent, comme les autres réglages) — `sms_api_url`/`sms_api_key`/`sms_prefix` de la v1.0.31 y ont aussi été ajoutés au passage (oubliés lors de cette session-là). Vérifié en local : inscription déclenche bien la tentative SMS (loggée en `SKIPPED` sans `sms_api_key` configurée, comme attendu) sans bloquer la création du compte. Compte de test supprimé après vérification.
### Récap v1.0.30 → v1.0.31 (2026-08-04)
- **v1.0.31** : admin → Paramètres, nouvelle section « 📱 SMS » pour se connecter à l'API Passerelle SMS (sms-web.fbc.fr/docs, auth Bearer). Trois nouveaux `settings` (aucune règle métier en dur, comme demandé) : `sms_api_url` (URL de la passerelle, défaut `https://sms-web.fbc.fr`), `sms_api_key` (clé API de type "web", champ masqué comme `smtp_pass`), `sms_prefix` (texte ajouté au début de chaque SMS). Nouveau `SmsService` (`backend/src/sms/sms.service.ts`, module `@Global()` comme `EmailModule`/`WhatsAppModule`) : sans `sms_api_key` configurée, le SMS est simplement loggé (même esprit que Email/WhatsApp). Bouton « Envoi de SMS de test » dans la section (numéro au choix), `POST /api/v1/admin/test-sms` (`backend/src/admin/admin-test-sms.controller.ts`, réservé ADMIN/SUPER_ADMIN) — messages d'erreur actionnables selon le code HTTP retourné par l'API (401 clé invalide, 403 clé révoquée/mauvais type, etc.), calqué sur le pattern `admin-test-email.controller.ts`. Pas de table de log dédiée (contrairement à `WhatsAppLog`) : non demandé, à ajouter plus tard si un vrai flux de notification SMS voit le jour. Vérifié en local : section admin affichée, sauvegarde des 3 réglages en base OK, bouton de test déclenche bien l'appel vers `sms-web.fbc.fr/api/v1/messages` avec les bons headers/payload (erreur 404 obtenue en local due au proxy sortant du sandbox de dev, pas un bug — à revérifier une fois déployé/en environnement avec accès réseau direct au domaine). Compte de test et valeurs de settings de test supprimés après vérification.
...
### Récap v1.0.29 → v1.0.30 (2026-08-03)
- **v1.0.30 (correctifs)** :
  - Messagerie : le bouton « Écrire au Monstre » doit contacter le compte `admin@fbc.fr` précisément, pas n'importe quel admin. `MessagesService.findSupportRecipient()` réutilise le setting existant `admin_notification_email` (déjà utilisé par `AuthService` pour notifier un admin des nouvelles inscriptions, défaut `admin@fbc.fr`) — recherche l'utilisateur par cet email en premier, ne retombe sur « SUPER_ADMIN le plus ancien puis ADMIN le plus ancien » que si aucun compte ne correspond à cet email. Respecte la règle d'or « aucune règle métier en dur » : pas de nouvel email codé en dur, réutilisation d'un setting déjà éditable depuis l'admin. Vérifié en local : sans compte `admin@fbc.fr` dans `dev.db`, retombe correctement sur le fallback (ADMIN le plus ancien) ; avec un compte `admin@fbc.fr` (SUPER_ADMIN) créé pour le test, l'endpoint `GET /api/v1/messages/support-recipient` le retourne bien en priorité. Compte de test supprimé après vérification.
  - **Interface obsolète qui persiste sur mobile (cache)** : la cause était le service worker PWA — il servait l'ancien shell (`index.html`) depuis son precache, et `sw.js` était mis en cache ~4 h par nginx (config déployée antérieure au correctif du repo), donc le navigateur ne vérifiait même pas une mise à jour ; vider le cache de l'appli ne suffisait pas car le SW vit dans le profil Chrome (mode Custom Tab, assetlinks en attente de déploiement). Correctif en 3 couches : (1) `sw.ts` — navigation **réseau d'abord** : à chaque ouverture, `index.html` est rechargé depuis le serveur (repli sur le shell précaché hors ligne) ; (2) `frontend/nginx.conf` — `no-cache` sur `sw.js`, `registerSW.js` et `index.html` (les assets hashés restent immuables 30 j) ; (3) alignement des versions `frontend`/`backend` en 1.0.30 pour que le rechargement auto « version obsolète » (v1.0.25) ne boucle plus. À appliquer : redéployer frontend **et** backend ensemble (docker compose), et sur le téléphone vidé de son cache, effacer aussi les données du site dans Chrome (ou réinstaller l'APK — même origine).
...
- **v1.0.24** : création de zones d'alertes directement sur la carte (clic pour centrer + rayon), bouton « Définir une zone d'alerte » dans `MapView.vue` (compte requis, sinon redirection connexion). Profil : section « Zones d'Alertes » mise en avant (carte dédiée, coordonnées par zone, bouton ajout plein), raccourci « Messages » en haut du profil retiré. Admin → Monstres : bouton « Éditer » + modale (titre, description, catégorie, latitude/longitude, adresse) via nouveau `PATCH /api/v1/admin/items/:id` + `UpdateAdminItemDto` (champs optionnels, `categoryId` nullable). Page `/apicat` : exemple cURL affiché même si catégories en échec + bouton « Réessayer ».
- **v1.0.23** : navigation simplifiée. Le bouton « Boîte » devient « Messages » (badge de non-lus limité aux conversations). Les alertes — renommées « Zones d'Alertes » — quittent l'inbox et sont embarquées dans l'onglet Profil (section zones surveillées + notifications). `InboxView.vue` supprimé, `/messages` charge `MessagesView.vue` directement, `/alertes` redirige vers `/profil#zones-alertes` (scroll automatique vers la section). Store `notifications.ts` supprimé (compteur d'alertes plus affiché en nav). Carte : nouveau bouton « Définir une zone d'alerte » sous la carte *(devenu en v1.0.24 une création sur la carte elle-même)*. Page cachée `/apicat` (liste catégories + exemple `POST /api/v1/items`).
- **v1.0.22** : boîte de réception unifiée. La BottomNav ne garde que 5 boutons : « Boîte » fusionne les anciens onglets Messages et Alertes, avec un badge de non-lus combiné (messages + notifications). Nouvelle vue `InboxView.vue` (onglets Messages/Alertes en haut), `GET /api/v1/notifications/unread-count` ajouté, store `notifications.ts` pour le compteur. `/alertes` redirige vers `/messages?tab=alertes`. MessagesView et AlertsView conservés tels quels, embarqués dans InboxView. *(InboxView, la redirection vers `/messages?tab=alertes` et le store notifications ont depuis été retirés en v1.0.23.)*
...
- **v1.0.2** : ressourceries affichées dans une popup (bouton « Trouve une déchèterie »), bouton « Y aller » ouvre l'app GPS native, cache position 2 min pour affichage instantané sur mobile. Bouton « Sauvegarder » ajouté aux textareas admin. Disclaimer par défaut pré-rempli.
