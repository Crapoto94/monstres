...
onMounted(async () => {
  try {
    const { data: res } = await api.get<ApiSuccess<{ content: string }>>('/legal/cgu')
    content.value = res.data.content
  } catch {
    error.value = 'Impossible de charger les conditions générales d\'utilisation.'
  } finally {
    loading.value = false
  }
})
...
<template>
  <section class="flex-1 p-4 pb-24">
    <div class="mb-4 flex items-center gap-2">
      ...
      <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Conditions Générales d'Utilisation</h1>
    </div>
